...
qc_tsv_arthropod_primary <- qc_tsv_arthropod_primary %>%
  mutate(category = case_when(
    ...
    pid %in% c(conflicting_arthropods_order$pid, conflicting_arthropods_family$pid) ~ 7,
    rep_count_primary > 200 & no_conflicting_sequences < 10 ~ 1,
    rep_count_primary >= 100 & rep_count_primary <= 200 & no_conflicting_sequences < 10 ~ 2,
    rep_count_primary < 100 & no_conflicting_sequences < 10 ~ 3,
    rep_count_primary > 200 & no_conflicting_sequences >= 10 ~ 4,
    rep_count_primary >= 100 & rep_count_primary <= 200 & no_conflicting_sequences >= 10 ~ 5,
    rep_count_primary < 100 & no_conflicting_sequences >= 10 ~ 6,
    TRUE ~ NA_real_  
  )) %>%
  mutate(category_explanation = case_when(
    category == 1 ~ "Over 200 primary reads and no secondary sequence with 10 or more reads",
    category == 2 ~ "100 - 200 primary reads and no secondary sequence with 10 or more reads",
    category == 3 ~ "Fewer than 100 primary reads and no secondary sequence with 10 or more reads",
    category == 4 ~ "Over 200 primary reads and secondary sequence with 10 or more readss",
    category == 5 ~ "100 - 200 primary reads and secondary sequence with 10 or more reads",
    category == 6 ~ "Fewer than 100 primary reads and secondary sequence with 10 or more reads",
    category == 7 ~ "Family or order level conflicts detected [50% or more reads]",
    category == 8 ~ "Fewer than 5 reads and secondary sequence with 3 or more reads",
