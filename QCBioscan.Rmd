...
qc_secondary %>% 
  ...
  geom_histogram(binwidth = 1, fill = "#07dbd0", color = "#1c67fc", linewidth = 0.5) +
  labs(title = "Number of reads [secondary sequences]", x = "Read count", y = "Frequency") +
  theme_classic() +
...
cat(paste("Read count mean of all secondary sequences in all samples:", mean(qc_secondary$rep_count_primary)))
cat(paste("Read count mean of all positive control sequences in other samples:", mean(p_cont_contamination$rep_count_primary)))
```
...
cat(paste("Number of samples with only primary chimeric contig recognised:", nrow(only_primary_chimera),
          "\nWe do not know how mBRAVE recognises chimeras - for now ony samples represented by 5 or less reads get removed\n\nRetained samples:"))
chimera_primary_retained <- chimera_primary %>% filter(assignment == "primary" & rep_count_primary > 5) %>% filter(pid %in% only_primary_chimera$pid) %>% dplyr::select(pid, rep_count_primary, id_similarity_primary, p_primary, c_primary, o_primary, f_primary, g_primary, s_primary) %>% arrange(-rep_count_primary)
...
# Samples to remove 
anopheles_samples_to_remove <- anopheles_samples_to_examine %>% filter(rep_count_primary >= 250) %>% pull(pid)
...
cat(paste("Number of African Anopheline samples:", length(anopheles_pids),
          "\nNumber of primary African Anopheline hits [250 or more reads]:", length(anopheles_samples_to_remove),
...
# Exclude everything that is 5 reads or more 
...
ggplot(pid_counts, aes(x = no_conflicting_sequences)) +
  ...
  labs(title = "Number of Arthropod Contigs per Sample [binwidth = 1]",
       x = "Number of Contigs",
       y = "Frequency") +
  theme_classic() +
  theme(
...
qc_tsv_arthropod_primary <- qc_tsv_arthropod_primary %>%
  group_by(pid) %>%
  mutate(
    # Step 3: Find conflicting secondary rows (those with a different f_primary)
    conflicting_secondary = if_else(
      assignment == "secondary" & f_primary != primary_f_value & f_primary != "None", 
      TRUE, 
      FALSE
    ),
    # Step 4: Find the maximum rep_count_primary from the conflicting secondary rows
    ...
    # Step 5: Assign this max_rep_count_conflict to all rows of the pid group
...
qc_tsv_arthropod_primary <- qc_tsv_arthropod_primary %>%
  group_by(pid) %>%
  mutate(
    # Step 3: Find conflicting secondary rows (those with a different o_primary)
    conflicting_secondary = if_else(
...
conflicting_arthropods <- conflicting_arthropods %>% dplyr::select(-UMI.Plate.ID)
...
# Remove chimeras with low read support CHANGE THIS!!!!!!
# Get only 5 or check if it's been done 
qc_tsv_arthropod_primary <- qc_tsv_arthropod_primary %>% filter((str_detect(otu_primary, "chimera") & rep_count_primary < 6))

cat(paste("Number of retained samples:", length(unique(qc_tsv_arthropod_primary$pid)),
          "\nNumber of Arthropod samples assigned by mBRAVE [this inscludes samples with 5 or less reads that have now been excluded!]:",
...
qc_tsv_arthropod_primary <- qc_tsv_arthropod_primary %>%
  mutate(category = case_when(
    ...
    rep_count_primary > 200 & tax_conflict_read_count < 5 ~ 6,
    rep_count_primary >= 50 & rep_count_primary <= 200 & tax_conflict_read_count < 5 ~ 7, # NO MORE THAN 4 CONFLICTING SECONDARY READS OF A SINGLE SEQUENCE 
    rep_count_primary > 200 & best_secondary_read_count < 5 ~ 4,
    rep_count_primary >= 50 & rep_count_primary <= 200 & best_secondary_read_count < 5 ~ 5, # NO MORE THAN 4 NON-CONFLICTING SECONDARY READS OF A SEQUENCE 
    ...
    TRUE ~ "no_category"  # To check if anything goes wrong with filtering 
  )) %>%
  mutate(category_explanation = case_when(
    category == 1 ~ "Only one sequence with more than 200 reads",
    category == 2 ~ "Only one sequence with 50 to 200 reads",
    category == 3 ~ "Only one sequence with more than 5 but less than 50 reads",
    category == 4 ~ "Dominant sequence with more than 200 reads, non-conflicting secondary sequences with less than 5 read support",
    category == 5 ~ "Dominant sequence with 50 to 200 reads, non-conflicting secondary sequences with less than 5 read support",
    category == 6 ~ "Dominant sequence with more than 200 reads, conflicting secondary sequences with less than 5 read support",
    category == 7 ~ "Dominant sequence with 50 to 200 reads, conflicting secondary sequences with less than 5 read support",
    category == 8 ~ "Dominant sequence with more than 200 reads, secondary sequences with 5 or more read support",
    category == 9 ~ "Dominant sequence with 50 to 200 reads, secondary sequences with 5 or more read support",
    category == 10 ~ "Dominant sequence with more than 5 but less than 50 reads, non-conflicting secondary sequences with less than 5 read support",
...
summary_merge <- summary_merge %>% 
  mutate(category_decision = case_when(
    Category %in% c(1, 2, 3) ~ "YES",
    Category %in% c(6, 7, 4, 5) ~ "NO",
    Category == 8 ~ "NO",
